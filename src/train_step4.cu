...
int main(int argc, char** argv) {
    ...
    std::mt19937 train_rng(42 + rank);
    std::mt19937 valid_rng(123 + rank);
    ...
    for (int step = start_step; step <= max_steps; ++step) {
        ...
        if (step % eval_every == 0 || step == 1) {
            float local_train_loss = average_loss_from_device(d_losses);

            float local_valid_loss = estimate_loss(
                valid_tokens,
            ...
            float global_valid_loss = 0.0f;
            MPI_Reduce(&local_train_loss, &global_train_loss, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
            MPI_Reduce(&local_valid_loss, &global_valid_loss, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);

            if (rank == 0) {
                std::cout << "step " << step
                          << " | train loss " << global_train_loss / world_size
                          << " | valid loss " << global_valid_loss / world_size
                          << std::endl;
            }
        }
    ...
    if (rank == 0) std::cout << "Training finished." << std::endl;
